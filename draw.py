...
def plot_candles(pricing=[], title='test.jpg',
    ...
    """ Plots a candlestick chart using quantopian pricing data.  #使用量子定价数据绘制烛光图
    ...
    pricing：熊猫数据帧，列['open_price'，'close_price，'high'，'low'，'volume']
    title：图表的可选标题
    volume_bars：如果为True，则打印体积栏
    color_function：给定行索引和价格序列，返回蜡烛颜色的函数。
    叠加：要叠加在定价之上的附加数据系列列表。长度必须与定价相同。
    技术性：显示为子图的附加数据系列列表。
    technicals_titles：为每个技术指标显示的标题列表。
    """
...
#data preprocessing
def data_generate(chart_data, range = 1, start_idx = 0, actions=None, title = 'test1.png', type =4):
    #chart_data = np.flip(np.genfromtxt(chart_data, delimiter=','), 0)
    if start_idx == 0:
        #range += 1
        ma5 = talib.SMA(chart_data[-(range+5):,1],5)[5:]
        ma10 = talib.SMA(chart_data[-(range+10):, 1], 10)[10:]
        #ma20 = talib.SMA(chart_data[-(range+20):, 1], 20)[20:]
        ...
        # ma20 = talib.SMA(chart_data[start_idx-(range+20):start_idx, 1], 20)[20:]
...
def data_load(file, range = 1, start_idx = 0):
    ...
    chart_data = np.flip(np.genfromtxt(file, delimiter=','), 0)
    if start_idx == 0:
        #range += 1
        ...
        # ma20 = talib.SMA(chart_data[-(range+20+start_idx):-start_idx, 1], 20)[20:]
    ...
    plot_candles(pdData, title='test2', overlays=overlay_chart, volume_bars=True, volume_overlay=overlay_volume, technicals=extra_data, bband=bband,technical_y=None)



if __name__ == '__main__':
    from time import sleep
    #data_load('zj.csv', 25, 0)
    data_generate('zj.csv', 25, 0)
    pass
