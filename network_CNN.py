    ...
    def __init__(self, model_type, is_train = True, data_type = 0, learning_rate = 0.01,name=None):
        ...
        with tf.device(device):
            with tf.variable_scope(name):
                ...
                img_result = tf.reshape(tf.transpose(self.batch_norm_5, perm=[0, 2, 1, 3]), shape=[-1,
                                                                                              self.batch_norm_5.shape[2] *
                                                                                              self.batch_norm_5.shape[1] *
                                                                                              self.batch_norm_5.shape[
                                                                                                  3]])  # (1,2816)
                ...
                rnn_out = slim.fully_connected(img_result, 1024, activation_fn=tf.nn.leaky_relu,
                                               ...
                                               biases_initializer=None)  #(1,1024)
